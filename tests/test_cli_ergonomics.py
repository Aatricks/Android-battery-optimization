...
class TestCliErgonomics(unittest.TestCase):
    ...
    def test_global_dry_run_after_subcommand(self):
        with patch("sys.exit") as mock_exit:
            main(["apply-safe", "--dry-run"])
            assert mock_exit.call_count == 0

    def test_global_serial_before_subcommand(self):
        with patch("sys.exit") as mock_exit:
            main(["--serial", "ABC", "status"])
            assert mock_exit.call_count == 0

    def test_global_serial_after_subcommand(self):
        with patch("sys.exit") as mock_exit:
            main(["status", "--serial", "ABC"])
            assert mock_exit.call_count == 0

    def test_restrict_apps_accepts_dry_run_after_subcommand(self):
        with patch("sys.exit") as mock_exit:
            main(["restrict-apps", "--level", "ignore", "--yes", "--dry-run"])
            assert mock_exit.call_count == 0

    def test_whitelist_accepts_state_dir_after_subcommand(self):
        with patch("sys.exit") as mock_exit:
